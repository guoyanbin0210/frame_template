 ...
 * @hard {Object} vars 创建imageUploader时所需要的参数
 * @hard {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @hard {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @hard {Number} vars.picWidth 单张预览图片的宽度
 * @hard {Number} vars.picHeight 单张预览图片的高度
 * @hard {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @hard {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @hard {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @hard {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @hard {Number} vars.maxNum:32 最大上传多少个文件
 * @hard {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @hard {String} vars.url 上传的url地址
 * @hard {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @hard {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @hard {String} createOptions.width
 * @hard {String} createOptions.height
 * @hard {Number} maxNum 最大可选文件数
 * @hard {Function|String} selectFile
 * @hard {Function|String} exceedMaxSize
 * @hard {Function|String} deleteFile
 * @hard {Function|String} uploadStart
 * @hard {Function|String} uploadComplete
 * @hard {Function|String} uploadError
 * @hard {Function|String} uploadProgress
 */
 ...
 * @hard {String} [charset] script的字符集
 * @hard {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @hard {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @hard {string} [charset] script的字符集
 * @hard {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @hard {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @hard {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
